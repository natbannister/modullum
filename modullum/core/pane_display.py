...
def close_extra_tmux_panes():
    """Kill all tmux panes except the current one."""
    try:
        subprocess.run(["tmux", "kill-pane", "-a"], check=True)
    except (subprocess.CalledProcessError, FileNotFoundError):
        pass

class PaneDisplay:
    """
    Prints output to a tmux split pane if available.
    Falls back to a callable (e.g. logger.info) or stdout.
    """
    ...
    def close(self):
        if self.autoclose and self._pane_id and self._in_tmux:
            if self.pause_before_close > 0:
                time.sleep(self.pause_before_close)
            subprocess.run(
